  ...
  <body>
    <div class="Card">
      <section class="layout">
            ...
            <div class="col d-flex flex-column justify-content-center align-items-center">
              ...
              <div class="card overflow-scroll">
                <table class="table table-hover  table-sm">
                  ...
                  <tbody>
                    <tr v-for="elm in filteredCarga" :key="elm.tarifa" @click="FunClick(elm.tarifa, elm.nombre, elm.tipo,  elm.status)">
                      <td>{{ elm.tarifa }}</td>
                      <td>{{ elm.nombre }}</td>
                      <td>{{ elm.nom_tipo }}</td>
        ...
        <UpTiTarifa v-if="btnUp" :tarifa="tarifa" :nombre="nombre" :tipo="tipo" :status="status" :btnUp="btnUp"
          @updateProps="updatePropsValue" />
        <InTiTarifa v-if="btnIn" :btnIn="btnIn"  @insertProps="insertPropsValue" />
        <DlTiTarifa v-if="btnDl" :btnDl="btnDl" :tarifa="tarifa" @deleteProps="deletePropsValue" />
      </section>
    </div>
  </body>
</template>
...
import InTiTarifa from './pInsert/InTiTarifa.vue';
import UpTiTarifa from './pUpdate/UpTiTarifa.vue';
 

const dUrl = UrlGlobal()

const cartitar = ref<Array<Cartitar>>([]);
const search = ref('')

const getTipo = () => {
  axios.get(dUrl.urlGlobal +'/api2/cartitar/')
    .then(response => {
      cartitar.value = response.data;
    })
    .catch(error => {
...
const filteredCarga = computed(() => {
  if (search.value === '') {
    ...
    return cartitar.value.filter(elm => {
      return (
        elm.tarifa?.toLowerCase().includes(searchTerm) ||
...
const FunClick = (n: any, nm: any, m: any, st: any) => {
  tarifa = n;
  nombre = nm;
  tipo = m;
  status = st;
