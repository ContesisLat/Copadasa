<template>
    ...
    <div class="ReportPage">
        <h4>Actualizar Tipos de Tarifas</h4>
        <hr>
        <form class="row g-3 needs-validation" novalidate>
            <div class="col-md-2">
                <label for="validationCustom01" class="form-label">Tarifa</label>
                <input type="text" class="form-control" id="validationCustom01" v-model="tarifa" required disabled>
...
const handleSubmit = async () => {
    try {
        const response = await axios.post(dUrl.urlGlobal +'/api2/update/', {
            table: 'cartitar', 
            filters: { tarifa: tarifa.value }, // Filtro para identificar el registro a actualizar
